...
def main():
    ...
    for feature_set in ["vader", "finbert"]:
        ...
        features_df = data_prep.load_features_from_db(
            feature_set_name=feature_set, target_db="neondb_production", min_samples=30
        )
        ...
        splits = data_prep.prepare_train_test_split(features_df, test_size=0.2, validation_size=0.1)
        ...
        results = trainer.train_all_models(
            X_train_scaled, splits["y_train"], X_val_scaled, splits["y_val"]
        )
        ...
        all_results[feature_set] = {
            ...
            "splits": splits,
            "scaled_data": {"X_test": X_test_scaled, "y_test": splits["y_test"]},
        }
        ...
        best_model_key = [k for k, v in results.items() if v["model_name"] == best_model_type][0]
        best_model = results[best_model_key]["model"]

        print(f"\n Evaluating best model ({best_model_type}) on test set...")
        test_metrics = trainer.evaluate_model(best_model, X_test_scaled, splits["y_test"])

        print(f"  Test Accuracy: {test_metrics['accuracy']:.4f}")
        print(f"  Test F1 Score: {test_metrics['f1_score']:.4f}")
        print(
            f"  Test ROC AUC: {test_metrics.get('roc_auc', 'N/A'):.4f}"
    ...
    if "vader" in all_results and "finbert" in all_results:
        if all_results["vader"]["model_results"] and all_results["finbert"]["model_results"]:
            # Generate comparison report
            comparison_df = comparator.compare_feature_sets(
                all_results["vader"]["model_results"], all_results["finbert"]["model_results"]
            )
            ...
            sig_test = comparator.statistical_significance_test(
                vader_pred, finbert_pred, all_results["vader"]["scaled_data"]["y_test"].values
            )
