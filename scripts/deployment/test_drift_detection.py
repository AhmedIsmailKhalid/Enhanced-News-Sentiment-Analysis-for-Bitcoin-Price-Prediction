...
def setup_test_data():
    ...
    try:
        ...
        historical_ids = []
        for i, (feature_set, model_type, pred, actual, price_start, price_end) in enumerate(
            test_predictions
        ):
        ...
        for pred_id, actual, price_end in historical_ids:
            prediction_logger.update_prediction_outcome(
                prediction_id=pred_id, actual_direction=actual, bitcoin_price_1h_later=price_end
            )
...
def test_feature_drift_detection():
    ...
    try:
        ...
        drift_results = drift_detector.detect_feature_drift(
            feature_set="vader", reference_days=7, current_days=1
        )
        ...
        if drift_results["status"] == "success":
            ...
            logger.warning(
                f"⚠️  {drift_results.get('message', 'Feature drift detection incomplete')}"
            )
...
def test_model_drift_detection():
    ...
    try:
        ...
        drift_results = drift_detector.detect_model_drift(
            feature_set="vader", model_type="random_forest", reference_days=7, current_days=1
        )
        ...
        if drift_results["status"] == "success":
            ...
            logger.info(
                f"  Reference Calibration Gap: {confidence.get('reference_calibration_gap', 0):.3f}"
            )
            logger.info(
                f"  Current Calibration Gap: {confidence.get('current_calibration_gap', 0):.3f}"
            )
            logger.info(
                f"  Calibration Degradation: {confidence.get('calibration_degradation', 0):.3f}"
            )
            ...
            logger.warning(
                f"⚠️  {drift_results.get('message', 'Model drift detection incomplete')}"
            )
            return True  # Not a failure, just insufficient data
...
def test_drift_summary():
    ...
    try:
        ...
        summary = drift_detector.get_drift_summary(feature_set="vader", model_type="random_forest")
