...
def test_get_recent_predictions():
    ...
    try:
        ...
        if predictions:
            ...
            logger.info(
                f"  Actual: {'UP' if pred['actual_direction'] == 1 else 'DOWN' if pred['actual_direction'] == 0 else 'Unknown'}"
            )
...
def test_get_model_accuracy():
    ...
    try:
        ...
        accuracy_stats = prediction_logger.get_model_accuracy(
            feature_set="vader", model_type="random_forest", days=7
        )
        ...
        if accuracy_stats.get("accuracy") is not None:
            ...
            logger.info(
                f"  UP Predictions: {accuracy_stats.get('up_predictions')} (accuracy: {accuracy_stats.get('up_accuracy'):.2%})"
            )
            logger.info(
                f"  DOWN Predictions: {accuracy_stats.get('down_predictions')} (accuracy: {accuracy_stats.get('down_accuracy'):.2%})"
            )
            logger.info(f"  Avg Confidence: {accuracy_stats.get('avg_confidence'):.3f}")
            logger.info(
                f"  Avg Confidence (Correct): {accuracy_stats.get('avg_confidence_when_correct'):.3f}"
            )
            logger.info(
                f"  Avg Confidence (Incorrect): {accuracy_stats.get('avg_confidence_when_incorrect'):.3f}"
            )
