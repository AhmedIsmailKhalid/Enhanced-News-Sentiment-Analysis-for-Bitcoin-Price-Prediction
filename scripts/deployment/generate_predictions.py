...
class UnifiedPredictionGenerator:
    ...
    def _load_models(self):
        ...
        try:
            ...
            self.logger.info(
                f"Loaded VADER model: {vader_best} (version: {self.vader_model['version']})"
            )
            ...
            self.logger.info(
                f"Loaded FinBERT model: {finbert_best} (version: {self.finbert_model['version']})"
            )
        ...
        for model_type_dir in models_dir.iterdir():
            ...
            try:
                ...
                if val_accuracy > best_accuracy or (
                    val_accuracy == best_accuracy and timestamp_str > best_timestamp
                ):
    ...
    def get_features_without_predictions(
        self, mode: str = "recent", hours: int = 1
    ) -> List[FeatureData]:
        """
        ...
        try:
            ...
            query = (
                ...
                .filter(FeatureData.feature_set_name == "vader", PredictionLog.id.is_(None))
            )
            ...
            self.logger.info(
                f"Found {len(features)} features without predictions " f"(mode: {mode})"
            )
    ...
    def generate_prediction(
        self, features: pd.Series, feature_set: str, model_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _prepare_features(self, features: pd.Series, expected_columns: List[str]) -> List[float]:
        """
    ...
    def process_features(
        self, features_list: List[FeatureData], batch_size: int = 50
    ) -> Dict[str, int]:
        """
            ...
            for vader_feature in batch:
                try:
                    ...
                    vader_result = self.generate_prediction(
                        vader_features, "vader", self.vader_model
                    )
                    ...
                    if finbert_feature:
                        ...
                        finbert_result = self.generate_prediction(
                            finbert_features, "finbert", self.finbert_model
                        )

                        if finbert_result:
                            if self.store_prediction(
                                "finbert", timestamp, finbert_result, bitcoin_price
                            ):
                                finbert_success += 1
...
def main():
    ...
    parser.add_argument(
        "--hours", type=int, default=1, help="For recent mode: how far back to look (hours)"
    )
    parser.add_argument(
        "--batch-size", type=int, default=50, help="Number of features to process per batch"
    )
    parser.add_argument("--db", default="neondb_production", help="Target database")
    ...
    try:
        ...
        features = generator.get_features_without_predictions(mode=args.mode, hours=args.hours)
        ...
        # Process features
        stats = generator.process_features(features, batch_size=args.batch_size)
