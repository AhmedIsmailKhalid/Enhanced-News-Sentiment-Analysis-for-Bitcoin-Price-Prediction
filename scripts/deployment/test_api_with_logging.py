...
def test_dual_prediction_with_logging():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Total Response Time: {data['performance']['total_response_time_ms']:.2f}ms")
            ...
            if vader_logged and finbert_logged:
                ...
                logger.warning(f"⚠️  Logging incomplete - VADER: {vader_logged}, FinBERT: {finbert_logged}")
                return False
        else:
...
def main():
    ...
    results.append(("Single Prediction with Logging", test_single_prediction_with_logging() is not None))
