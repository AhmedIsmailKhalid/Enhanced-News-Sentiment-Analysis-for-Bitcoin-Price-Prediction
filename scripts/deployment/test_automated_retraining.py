...
def test_retraining_decision():
    ...
    try:
        ...
        decision = retrainer.should_retrain(feature_set="vader", model_type="random_forest")
...
def test_both_feature_sets_retraining():
    ...
    try:
        ...
        results = retrainer.retrain_both_feature_sets(
            model_type="random_forest", deploy_if_better=False
        )
...
def test_threshold_configuration():
    ...
    try:
        ...
        logger.info(
            f"  - Retraining triggers if accuracy drops by {retrainer.accuracy_degradation_threshold:.0%}"
        )
        logger.info(
            f"  - Retraining triggers if drift severity is '{retrainer.drift_severity_threshold}' or higher"
        )
        logger.info(f"  - At least {retrainer.min_samples_required} samples needed for retraining")
        logger.info(
            f"  - At least {retrainer.min_prediction_count} predictions needed for performance evaluation"
        )
...
def test_performance_check():
    ...
    try:
        ...
        performance = retrainer._check_performance_degradation(
            feature_set="vader", model_type="random_forest"
        )
