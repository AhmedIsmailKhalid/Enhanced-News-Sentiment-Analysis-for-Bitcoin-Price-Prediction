...
def main():
    ...
    try:
        ...
        price_cols = [col for col in vader_dataset.columns if any(x in col for x in 
                     ['price', 'return', 'sma', 'rsi'])][:5]
        print(f"  {price_cols}")
        
        print("\n→ Sample Temporal features (shared by both):")
        temporal_cols = [col for col in vader_dataset.columns if any(x in col for x in 
                        ['hour', 'day', 'is_'])][:5]
        print(f"  {temporal_cols}")
        
        # Check for NaN values
        print("\n→ Data Quality Check:")
        vader_nan_pct = (vader_dataset.isnull().sum().sum() / (vader_dataset.shape[0] * vader_dataset.shape[1])) * 100
        finbert_nan_pct = (finbert_dataset.isnull().sum().sum() / (finbert_dataset.shape[0] * finbert_dataset.shape[1])) * 100
        ...
        finbert_dataset.head(100).to_csv('data/ml_datasets/finbert_features_sample.csv', index=False)
        ...
        print(f"  - VADER dataset: {vader_dataset.shape[0]} rows × {vader_dataset.shape[1]} features")
        print(f"  - FinBERT dataset: {finbert_dataset.shape[0]} rows × {finbert_dataset.shape[1]} features")
