...
class FeatureStorageManager:
    ...
    def store_feature_sets(
        self, vader_df: pd.DataFrame, finbert_df: pd.DataFrame, target_db: str = "local"
    ) -> Dict[str, int]:
        """
        ...
        try:
            results = {"vader_stored": 0, "finbert_stored": 0}

            # Store VADER features
            if not vader_df.empty:
                vader_count = self._store_feature_set(db, vader_df, feature_set_name="vader")
            ...
            if not finbert_df.empty:
                finbert_count = self._store_feature_set(db, finbert_df, feature_set_name="finbert")
    ...
    def _store_feature_set(self, db: Session, df: pd.DataFrame, feature_set_name: str) -> int:
        ...
        for idx, row in df.iterrows():
            ...
            existing = (
                ...
                .filter_by(feature_set_name=feature_set_name, timestamp=timestamp)
                .first()
            )
