...
class PriceFeatureEngineer:
    ...
    def create_features(self, price_df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['return_24h'] = df['price_usd'].pct_change(periods=1)  # Using 1 period for now (will be 24 with hourly data)
        ...
        feature_cols = ['collected_at', 'symbol', 'price_usd', 'return_24h', 'volatility_24h', 
                        'volume_24h', 'rsi_14', 'sma_7']
        df = df[[col for col in feature_cols if col in df.columns]]
        
        self.logger.info("Created 6 price features: price_usd, return_24h, volatility_24h, volume_24h, rsi_14, sma_7")
        return df
