...
class DataPreparation:
    ...
    def load_features_from_db(
        self, feature_set_name: str, target_db: str = "local", min_samples: int = 30
    ) -> pd.DataFrame:
        """
    ...
    def prepare_train_test_split(
        ...
        exclude_cols = [
            ...
            "symbol",
            "name",
            "data_source",  # Add these non-numeric columns
        ]
    ...
    def scale_features(
        ...
        X_train_scaled = pd.DataFrame(
            self.scaler.transform(X_train_imputed), columns=X_train.columns, index=X_train.index
        )

        X_val_scaled = pd.DataFrame(
            self.scaler.transform(X_val_imputed), columns=X_val.columns, index=X_val.index
        )

        X_test_scaled = pd.DataFrame(
            self.scaler.transform(X_test_imputed), columns=X_test.columns, index=X_test.index
        )
