...
class ModelComparator:
    ...
    def compare_feature_sets(
        self, vader_results: Dict[str, Any], finbert_results: Dict[str, Any]
    ) -> pd.DataFrame:
        """
        ...
        for model_type in vader_results.keys():
            ...
            row = {
                ...
                "accuracy_diff": (
                    finbert_res["val_metrics"]["accuracy"] - vader_res["val_metrics"]["accuracy"]
                ),
                "f1_diff": (
                    finbert_res["val_metrics"]["f1_score"] - vader_res["val_metrics"]["f1_score"]
                ),
            }
        ...
        comparison_df["winner"] = comparison_df.apply(
            lambda x: "FinBERT"
            if x["accuracy_diff"] > 0
            else ("VADER" if x["accuracy_diff"] < 0 else "Tie"),
            axis=1,
        )
    ...
    def statistical_significance_test(
        self, vader_predictions: np.ndarray, finbert_predictions: np.ndarray, y_true: np.ndarray
    ) -> Dict[str, Any]:
        """
    ...
    def generate_comparison_report(
        ...
        report = {
            ...
            "test_performance": {"vader": vader_test_metrics, "finbert": finbert_test_metrics},
        }
